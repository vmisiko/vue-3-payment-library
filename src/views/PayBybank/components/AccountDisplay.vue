      ...
      <div class="mgt-1">
        ...
        > 
          <template v-for="(obj, index) in accounts" >
            <option :key="index"  :value="obj.account_number"> {{ obj.bank_name }} </option>
          </template>
        </select>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    selectedAccount() {
      const bank = this.accounts.filter(el => el.account_number === this.account);
      return bank[0];
    }
  },
  watch: {
  ...
  methods: {
    ...
    handleCopy() {
      ...
      cb.writeText(span.innerText).then(() => {
        this.$paymentNotification({ text: 'Copied...'})
       })
    }
  }
}
